...
/// Returns the storage slot for a given address in a mapping(address => value)
/// at the specified mapping index (slot position in contract storage).
/// 
/// Formula: keccak256(abi.encodePacked(
///     left_padded(address, 32),
///     left_padded(mapping_index, 32)
/// ))
pub fn get_storage_location_for_key(
    ...
    // Address padded left (12 zero bytes + 20-byte address)
    encoded[12..32].copy_from_slice(address.as_slice());

    // Mapping index padded to 32 bytes
